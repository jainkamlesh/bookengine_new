...
<section class="property-wrapper mt-4">
      ...
      <div class="d-flex justify-content-between align-items-center container-fluid mt-4 pro-add-nnb">
        ...
        <a href="{{route('admin.add.currency')}}"><button class="btn-add add-btn"><i class="fa fa-plus" aria-hidden="true"></i>{{ __('label.addcurrency') }}</button></a>
      </div>
      <div class="container-fluid">
        ...
        <table class="mt-4 pro-table-pg table-hover wrapper">
          <thead>
            <tr class="ft-tr">
              <th></th>
              <th>{{ __('label.currencyname') }}</th>
              <th>{{ __('label.currencycode') }}</th>
              <th style="width: 250px;">{{ __('label.currencysymbol') }}</th>
        ...
        <div class="modal fade" id="promocodedata_{{$value->id}}">
          <div class="modal-dialog modal-dialog-centered promocode_main" role="document">
              ...
              <div class="modal-body">
                <div class="coupon">
                  ...
                  <div class="mt-4 modal-btn">
                    <a href="{{route('admin.currency.delete', ['id'=>$value->id])}}" class="btn btn-default btn-success" >{{ __('label.yes') }}</a>
            ...
            @else
            <tr>
              <td colspan="5" class="text-center" >{{ __('label.norecordfound') }}...</td>
            </tr>
            @endif
      ...
      <div class="pagination-btn text-right">
        <a class=" btn-add add-btn" href="{{ $currency->previousPageUrl() }}">{{ __('label.previous') }}</a>
        <a class=" btn-add add-btn" href="{{ $currency->nextPageUrl() }}">{{ __('label.next') }}</a>
      </div>
      @endif
