...
@section('content')
    <section class="property-wrapper mt-4">
        <a href="{{route('admin.coupon')}}" class="back-icon" data-toggle="tooltip" title="Back"><img src="{{url('/')}}/images/chevron.png" class="back-image"></a>
        <h2 class="list-heading">{{ __('user.editcoupon') }}</h2>
    </section>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-sm-3 mt-3 ">
                        ...
                        <input type="text" class="form-control text_float" name="discount_percentage" value="{!! old('discount_percentage', $coupon->discount_percentage) !!}"
                               onkeypress="return isNumber(event)">
                    </div>
                    <div class="col-sm-3 mt-3 ">
                        <label>{{ __('user.fixeddiscount') }} :</label>
                        <input type="text" class="form-control text_float" name="fixed_discount" value="{!! old('fixed_discount', $coupon->fixed_discount) !!}"
                               onkeypress="return isNumber(event)">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-6 mt-3 ">
                        ...
                        <input type="date" class="form-control coupon_date" name="valid_from" value="{!! old('valid_from', $coupon->valid_from) !!}">
                    </div>
                    <div class="col-sm-6 mt-3 ">
                        <label>{{ __('user.validto') }} :</label>
                        <input type="date" class="form-control coupon_date" name="valid_to" value="{!! old('valid_to', $coupon->valid_to) !!}">
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    jQuery('input[name="valid_from"]').on('apply.daterangepicker', function(ev, picker) {
        jQuery('#valid_from').val(picker.startDate.format('DD/MM/YYYY'));
    });
    ...
    jQuery('input[name="valid_to"]').on('apply.daterangepicker', function(ev, picker) {
        jQuery('#valid_to').val(picker.startDate.format('DD/MM/YYYY'));
    });
</script>
@endsection
